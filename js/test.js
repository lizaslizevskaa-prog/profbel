...
const testApp = createApp({
  data() {
    return {
      ...
      // 10 Вопросов с привязкой баллов к категориям
      questions: [
        {
  ...
  methods: {
    ...
    selectAnswer(answer) {
      // Сохраняем историю (на случай кнопки Назад)
      ...
      // Если тест окончен, считаем результат
    ...
    prevStep() {
      this.step--;
      // Отменяем баллы за последний ответ
    ...
    calculateResult() {
      ...
      top3Categories.forEach((category) => {
        ...
        if (prof) {
          ...
          // Fallback, если в категории нет свободных, берем просто любую следующую свободную
    ...
    saveResults() {
      ...
      // Получаем текущие сохраненные результаты из LocalStorage
      let pastResults =
      ...
      // Добавляем в начало и оставляем только последние 3
...
// Запускаем приложение Vue, если мы на странице теста
