document.addEventListener("DOMContentLoaded", () => {
  ...
  // ОСНОВНАЯ ФУНКЦИЯ ФИЛЬТРАЦИИ (ИСПРАВЛЕННАЯ)
  function applyFilters() {
    ...
    // 3. Образование (Безопасное получение значения)
    const eduChecked = document.querySelector(
      'input[name="eduFilter"]:checked',
    );
    const eduVal = eduChecked ? eduChecked.value : "Все";
    if (eduVal !== "Все") {
    ...
    // 5. Зарплата (Безопасное сравнение чисел)
    filtered = filtered.filter((p) => {
      // Превращаем строки из базы в числа
      const pMin = parseInt(p.salaryMin) || 0;
      const pMax = parseInt(p.salaryMax) || 5000;

      const matchMin = pMax >= minSalary;
      const matchMax = maxSalary >= 5000 ? true : pMin <= maxSalary;
      return matchMin && matchMax;
    });

    // 6. Востребованность (Безопасное получение значения)
    const demandChecked = document.querySelector(
      'input[name="demandFilter"]:checked',
    );
    const demandVal = demandChecked ? demandChecked.value : "Все";
    if (demandVal !== "Все") {
  ...
  // Сброс фильтров (ИСПРАВЛЕННЫЙ)
  resetBtn.addEventListener("click", () => {
    searchFilter.value = "";
    directionFilter.value = "Все";

    // Безопасно находим радиокнопки "Все"
    const eduAll = document.querySelector(
      'input[name="eduFilter"][value="Все"]',
    );
    if (eduAll) eduAll.checked = true;

    const demandAll = document.querySelector(
      'input[name="demandFilter"][value="Все"]',
    );
    if (demandAll) demandAll.checked = true;
